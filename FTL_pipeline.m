% Matlab script for illustrating the secondary structure of FTL's 5'UTR
% Define sequence
FTL_seq = 'gcaguucggcggucccgcgggucugucucuugcuucaacaguguuuggacggaacagauccggggacucucuuccagccuccgaccgcccuccgauuuccucuccgcuugcaaccuccgggaccaucuucucggccaucuccugcuucugggaccugccagcaccguuuuugugguuagcuccuucuugccaaccaacc';
...
RNAbracket = rnafold(FTL_seq, 'MinLoopSize', 7);
...
%%
% Define structure
FTL_struct = rnafold(FTL_seq);
% Mutate every position in the RNA with every possible nucleotide
mut_table = cell(length(FTL_seq)*4, 6);
h = waitbar(0, 'Generating mutation table...');
for i = 1:length(FTL_seq)
    waitbar(i/length(FTL_seq), h, sprintf('Mutating position %d of %d...', i, length(FTL_seq)));
    for n = 1:4
        mutated_seq = FTL_seq;
        if n == 1
            mutated_seq(i) = 'a';
        ...
        score = score_comp(FTL_struct, mutated_struct);
        ...
        row = {i, FTL_seq(i), nucleotide, score, original_pairdness, mutated_pairdness};
        mut_table(i*4+n, :) = row; % Note the change in this line
    end
end
close(h);
% Save table to a tab-separated file
fid = fopen('FTL_gsgeorge.txt', 'w');
...
h = waitbar(0, 'Calculating FTL structure...');
...
wild_type_scores = [score_comp(FTL_struct, wild_type_1), score_comp(FTL_struct, wild_type_2), score_comp(FTL_struct, wild_type_3)];
hold on;
...
set(gca, 'xticklabel', {'Paired to Unpaired', 'Unpaired to Paired', 'No Change', 'Overall'});
 ...
 %% This is commented out because it is unneeded amino acid analysis, however it is too pretty of a matlab script to delete!
% %Analysis of the amino acid structure of these sequences
% % Read the FTL sequence
% FTL_seq = 'gcaguucggcggucccgcgggucugucucuugcuucaacaguguuuggacggaacagauccggggacucucuuccagccuccgaccgcccuccgauuuccucuccgcuugcaaccuccgggaccaucuucucggccaucuccugcuucugggaccugccagcaccguuuuugugguuagcuccuucuugccaaccaacc';
% 
% % Read the reference sequence
% REF_seq = 'gcaguucggcggucccgcgggucugucucuugcuucaacaguguuuggacggaacagauccggggacucucuuccagccuccgaccgcccuccgauuuccucuccgcuugcaaccuccgggaccaucuucucggccaucuccugcuucugggaccugccagcaccguuuuugugguuagcuccuucuugccaaccaacc';
% 
...
%% Prior to parallel reconstruction of Mut Table as a matrix, clear variables from Environment 
clearvars;
% Define sequence
FTL_seq = 'gcaguucggcggucccgcgggucugucucuugcuucaacaguguuuggacggaacagauccggggacucucuuccagccuccgaccgcccuccgauuuccucuccgcuugcaaccuccgggaccaucuucucggccaucuccugcuucugggaccugccagcaccguuuuugugguuagcuccuucuugccaaccaacc';
...
% Calculate similarity score for every single nucleotide change to FTL_seq
score_matrix = zeros(4,length(FTL_seq));
parfor i = 1:length(FTL_seq)
    for j = 1:4
        temp_seq = FTL_seq;
        if j == 1
            temp_seq(i) = 'A';
        ...
        score_matrix(j,i) = score_comp(temp_struct, FTL_struct);
    end
end
